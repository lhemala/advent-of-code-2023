...
#[allow(unused)]
pub fn aoc_02_2(input: &str) {
    println!("aoc_02_2: {}", main(input, 12, 13, 14))
}

fn main(input: &str, _red: i64, _green: i64, _blue: i64) -> i64 {
    let (_, games) = separated_list1(tag("\n"), Game::parse)(input).unwrap();

    games.into_iter().map(|game| game.get_product()).sum()
}
...
mod test {
    ...
    #[test]
    fn test() {
        assert_eq!(main(INPUT, 12, 13, 14), 2286);
    }
}
