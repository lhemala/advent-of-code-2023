...
#[allow(unused)]
pub fn aoc_05_2(input: &str) {
    println!("aoc_05_2: {}", main(input))
}

fn main(input: &str) -> i64 {
    let (_, almanac) = Almanac::parse(input).unwrap();

    let seeds_len = almanac.seeds.len();
    println!("seeds.len: {seeds_len}");
...
impl Almanac {
    fn parse(input: &str) -> IResult<&str, Self> {
        ...
        let (input, ranges) =
            separated_list1(space1, separated_pair(complete::i64, space1, complete::i64))(input)?;
