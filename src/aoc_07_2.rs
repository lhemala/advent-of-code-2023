...
#[allow(unused)]
pub fn aoc_07_2(input: &str) {
    println!("aoc_07_2: {}", main(input))
}
...
impl From<HashMap<Card, usize>> for Type {
    fn from(mut input: HashMap<Card, usize>) -> Self {
        let jokers = input.get(&Card::Jack).cloned().unwrap_or_default();
        input.remove(&Card::Jack);

        if jokers == 5 || Self::has_kind(&input, 5 - jokers) {
            Self::FiveOfAKind
        } else if Self::has_kind(&input, 4 - jokers) {
            Self::FourOfAKind
        } else if Self::is_full_house(&input, jokers) {
            Self::FullHouse
        } else if Self::has_kind(&input, 3 - jokers) {
            Self::ThreeOfAKind
        } else if Self::get_count_of_kind(&input, 2 - jokers) == 2 {
            Self::TwoPair
        } else if Self::has_kind(&input, 2 - jokers) {
            Self::OnePair
        } else {
            Self::HighCard
        }
...
// 251421071
impl Type {
    fn is_full_house(input: &HashMap<Card, usize>, jokers: usize) -> bool {
        let has_3_of_a_kind = Self::has_kind(input, 3);
        let has_2_of_a_kind = Self::has_kind(input, 2);

        if has_3_of_a_kind && has_2_of_a_kind {
            return true;
        }

        if jokers == 1 && Self::get_count_of_kind(input, 2) == 2 {
            return true;
        }

        if jokers >= 2 && (has_2_of_a_kind || has_3_of_a_kind) {
            return true;
        }
